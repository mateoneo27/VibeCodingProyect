...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-[#f6f5fa] flex flex-col">
      <header className="sticky top-0 w-full z-50 bg-[#f6f5fa]/70 backdrop-blur-xl shadow-[0_12px_32px_rgba(25,28,33,0.06)]">
        <div className="flex justify-between items-center px-8 py-4">
          <div className="text-2xl font-black tracking-tighter text-[#0a29cd] font-headline">Neo</div>
        </div>
      </header>
      ...
      <main className="flex-grow flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            <div className="inline-flex items-center px-4 py-2 bg-[#b3b3c2] text-[#0e1c2e] font-label text-xs font-semibold uppercase tracking-widest rounded-full mb-4">
              NUEVO INGRESO
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight text-[#000033] font-headline mb-3">
              Crea tu cuenta
            </h1>
            <p className="text-[#666685]">Regístrate para iniciar tu proceso de onboarding.</p>
          </div>
          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-[0_12px_32px_rgba(25,28,33,0.06)]">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-[#666685] px-1">Contraseña</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Mínimo 6 caracteres"
                  className="w-full bg-[#ccccd6] border-none rounded-xl px-4 py-4 focus:ring-2 focus:ring-[#0a29cd] focus:bg-white transition-all outline-none" />
              </div>
              <div className="space-y-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-[#666685] px-1">Confirmar contraseña</label>
              ...
              <button type="submit" disabled={loading}
                className="w-full bg-gradient-to-b from-[#0a29cd] to-[#0a29cd] text-white font-headline font-bold rounded-2xl py-4 shadow-lg hover:opacity-90 active:scale-95 transition-all disabled:opacity-60">
                {loading ? 'Creando cuenta...' : 'Crear cuenta'}
              </button>
            </form>

            <p className="text-center text-sm text-[#666685] mt-6">
              ¿Ya tienes cuenta?{' '}
              <Link to="/auth/login" className="text-[#0a29cd] font-bold hover:underline">Inicia sesión</Link>
            </p>
          </div>
        </div>
      </main>
