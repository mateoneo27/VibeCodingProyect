              ...
              <div className="flex flex-col items-center">
                ...
                <div className="w-full mt-6 grid grid-cols-2 gap-3">
                  {[['Formato', 'JPG'], ['Tamaño', '340x402 px'], ['Peso', '25-30 KB'], ['Fondo', 'Blanco']].map(([k, v]) => (
                    <div key={k} className="bg-white p-3 rounded-xl border border-[#c2c6d4]/20">
                      <p className="text-[10px] uppercase font-bold text-[#58657a] tracking-wider mb-1">{k}</p>
                      <p className="text-xs font-semibold text-[#191c21]">{v}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-[#f2f3fb] rounded-[2rem] p-10 space-y-10">
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-[#c2c6d4]/20">
                    <span className="text-xs font-bold text-[#58657a] uppercase tracking-wider block mb-1">Dirección</span>
                    <p className="text-[#191c21] font-semibold">{dp.direccion || '—'}</p>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white p-5 rounded-xl flex items-center gap-4 border border-[#c2c6d4]/20">
                    <span className={`material-symbols-outlined ${state.beneficioEPS.desea ? 'text-[#005149]' : 'text-[#727783]'}`} style={{ fontVariationSettings: "'FILL' 1" }}>
                      {state.beneficioEPS.desea ? 'check_circle' : 'cancel'}
                    </span>
                    <div>
                      <p className="font-bold text-sm text-[#191c21]">EPS Rímac</p>
                      <p className="text-xs text-[#424752] italic">{state.beneficioEPS.desea ? 'Plan Base — Afiliado' : 'No seleccionado'}</p>
                    </div>
                  </div>
                  <div className="bg-white p-5 rounded-xl flex items-center gap-4 border border-[#c2c6d4]/20">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#424752] italic">{state.oncosalud.desea ? 'Cobertura Integral' : 'No seleccionado'}</p>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-[#c2c6d4]/20">
                  <span className="text-xs font-bold text-[#58657a] uppercase tracking-wider block mb-1">Fecha Sugerida</span>
                  <p className="text-[#191c21] font-semibold">{state.examenMedico.fechaSugerida || '—'}</p>
                </div>
              </section>
            </div>
          </div>
